...
from tests.unit.test_oauth_middleware import create_test_jwt
...
def isolated_mcp_app_client(monkeypatch, jwks_keys):
    ...
    with (
        ...
        mock_get_jwks.return_value = {"keys": [main_public_jwk_dict]}
        ...
        with TestClient(app) as client:
            yield client, main_private_pem, mock_cfg
...
def test_e2e_authenticated_tool_invocation(isolated_mcp_app_client, monkeypatch):
    ...
    """
    client, private_key_pem, mock_config = (
        isolated_mcp_app_client  # Changed private_key to private_key_pem
    )
    ...
    token = create_test_jwt(private_key_pem, issuer, "test-client")  # Used private_key_pem

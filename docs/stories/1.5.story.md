...
**Documentation Fix: Claude.ai URL Configuration (November 24, 2025)**

**Issue Found:**
- README.md incorrectly stated: "Use the full HTTPS URL without any path (no `/mcp` suffix needed)"
- User testing revealed Claude.ai requires the full path including `/mcp`
- Without `/mcp` path, Claude.ai cannot discover or connect to MCP server

**Correction Applied:**
- Updated README.md Step 3 with correct URL format
- **Correct URL:** `https://your-tunnel-url.trycloudflare.com/mcp` (WITH `/mcp`)
- Added "CRITICAL" note that `/mcp` path is mandatory
- Updated `.env` example to include `/mcp` in CLOUDFLARE_TUNNEL_URL
...
**Files Modified:**
- `README.md` - Corrected Claude.ai URL configuration instructions (line 143-144)
- `.env` - Updated with correct tunnel URL including `/mcp` path
